...
import static com.example.user.capstone.map.MathUtil.EARTH_RADIUS;
import static com.example.user.capstone.map.MathUtil.hav;
import static com.example.user.capstone.map.MathUtil.havDistance;
import static com.example.user.capstone.map.MathUtil.havFromSin;
import static com.example.user.capstone.map.MathUtil.inverseMercator;
import static com.example.user.capstone.map.MathUtil.mercator;
import static com.example.user.capstone.map.MathUtil.sinFromHav;
import static com.example.user.capstone.map.MathUtil.sinSumFromHav;
import static com.example.user.capstone.map.MathUtil.wrap;
import static com.example.user.capstone.map.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
            ...
            for (LatLng point2 : poly) {
                ...
                if (max(lat1, lat2) >= minAcceptable && min(lat1, lat2) <= maxAcceptable) {
                    ...
                    for (double x3 : xTry) {
                        ...
                        double t = len2 <= 0 ? 0 : MathUtils.clamp((x3 * x2 + (y3 - y1) * dy) / len2, 0, 1);
     ...
     */
    public static List<LatLng> douglasPeuckerReduction(List<LatLng> poly, double tolerance) {
